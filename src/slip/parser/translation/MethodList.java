...
class MethodList  // 20.2.2005 Début : 15:00 Fin : ???
{
  ...
  {
    dispatchAddress = LMAStmt.curAddress ;
    LMAStmt.curAddress += 4*(maxlevel + 1) ;
  ...
  void makeDispatchArray()
  {
    LMALit.curAddress = dispatchAddress + 4*(maxlevel + 1);
    int i = maxlevel + 1 ;
    while (i != 0)
    {
      i--;
      if (ma[i]==null) 
         new ILit(0, "Pas de méthode " + first.im.m + "/" +
                    i + "(" + first.im.arity + ")") ;
      else             
         new ILit(ma[i].entry, "Adresse de " + ma[i].m + "/" +
                    ma[i].level + "(" + ma[i].arity + ")") ;
    }
  }
